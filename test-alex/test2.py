...
def generate_revised_article(html_content, user_feedback, initial_request):
...
def main():
    ...
    if st.button("Generate SEO Article"):
        if not transcript:
            ...
            st.write(initial_article, unsafe_allow_html=True)
            st.download_button("Download Initial Article", initial_article, file_name="initial_article.html")
        else:
            initial_article, initial_request = generate_seo_article(transcript, target_languages)
            st.session_state['initial_article'] = initial_article
            st.session_state['initial_request'] = initial_request
    ...
    if 'initial_article' in st.session_state:
        ...
        st.write(st.session_state['initial_article'], unsafe_allow_html=True)
        ...
        if st.button("Submit Feedback"):
            if 'initial_request' in st.session_state and user_feedback:
                revised_article = generate_revised_article(
                    st.session_state['initial_article'], user_feedback, st.session_state['initial_request']
                )
                st.session_state['revised_article'] = revised_article
                st.subheader("Revised SEO Article")
                st.write(revised_article, unsafe_allow_html=True)
                st.download_button("Download Revised Article", revised_article, file_name="revised_article.html")
            else:
