...
def initialize_session_state():
    ...
    if 'neuronwriter_keywords' not in st.session_state:
        st.session_state['neuronwriter_keywords'] = None
...
def get_neuronwriter_keywords(article_content, existing_h1):
    ...
    new_query_payload = json.dumps({
        "project": os.getenv("NEURONWRITER_PROJECT_ID"),
        "keyword": existing_h1,
        "engine": "google.fr",
        "language": "French",
    })
...
def revise_article_with_neuronwriter(article_content, target_languages, model_name):
    """Generates an improved version of the article based on NeuronWriter recommendations."""

    # Get NeuronWriter keywords
    neuronwriter_keywords = get_neuronwriter_keywords(article_content, st.session_state['existing_h1'])

    if neuronwriter_keywords:
        neuronwriter_feedback = f"""
        Basic terms: {', '.join(neuronwriter_keywords['basic_terms'])}
        Terms with usage ranges: {', '.join(neuronwriter_keywords['terms_with_ranges'])}
        Entities: {', '.join(neuronwriter_keywords['entities'])}
        Recommended word count: {neuronwriter_keywords['word_count']}
        """
    else:
        neuronwriter_feedback = "NeuronWriter recommendations could not be retrieved."
        st.warning("Failed to retrieve NeuronWriter recommendations. The article will be revised without them.")

    system_revision = f"""
    You are an AI assistant skilled at revising SEO-optimized articles based on NeuronWriter recommendations. Follow these guidelines:

    - Use the NeuronWriter recommendations to optimize the content:
      - Include the recommended basic terms and entities where appropriate.
      - Aim for the suggested word count.
      - Pay attention to the suggested usage ranges for terms.
    - Ensure the revised article maintains its original structure and main points.
    - The revised content should be coherent, informative, and SEO-optimized.

    Current article content:
    {article_content}

    NeuronWriter recommendations:
    {neuronwriter_feedback}
    ...
    phospho.log(
        input=system_revision,
        output=revised_article_content,
        metadata={"task": "revise_article_with_neuronwriter"}
    )
...
if st.button("Generate SEO Article"):
    ...
    st.session_state['current_article'] = initial_article
...
if 'initial_article' in st.session_state and st.session_state['initial_article']:
    ...
    with col2:
        ...
        if st.button("Optimize with NeuronWriter"):
            ...
            st.session_state['current_article'] = optimized_article
